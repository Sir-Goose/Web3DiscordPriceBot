...
def get_any_price(command, dict_from_csv):
    ...
    if commandArray[1] == "imp" or commandArray[0] == "$imp" or commandArray[0] == "$IMP":
        ...
        impPrice = round(impPrice, 2)
        tokenFormattedPrice = "```" + "IMP $" + str(impPrice) + "```"
        return tokenFormattedPrice

    else:
        print(dict_from_csv[commandArray[1]])
        # convert ticker to token id used by coingecko
        tokenId = dict_from_csv[commandArray[1]]
        print(tokenId)

        tokenPriceArray = cg.get_price(ids=tokenId, vs_currencies='usd', include_24hr_change='true')
        ...
        tokenFormattedPrice = "```" + tokenId.upper() + " $" + str(tokenPrice) + " (" + str(
        tokenChange) + "%" + " 24h" + ")" + "```"
